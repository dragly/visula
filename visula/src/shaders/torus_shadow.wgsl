...
fn torus_shadow_vertex(
    ...
    output.input_color = vec3<f32>(0.0);
    output.torus_up = up;
...
fn vs_main(
    ...
    return torus_shadow_vertex(vertex_position, torus_geometry);
}
...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    let ray_dir_world = normalize(u_light.direction);

    let oc = in.vertex_position - in.torus_center;
    ...
    let od = dot(local_origin, local_dir);
    let oo = dot(local_origin, local_origin);
    let dd = dot(local_dir, local_dir);
    let k = oo - R2 - r2;

    let a4 = dd * dd;
    let a3 = 4.0 * dd * od;
    let a2 = 2.0 * dd * k + 4.0 * od * od + 4.0 * R2 * local_dir.y * local_dir.y;
    let a1 = 4.0 * od * k + 8.0 * R2 * local_origin.y * local_dir.y;
    let a0 = k * k + 4.0 * R2 * local_origin.y * local_origin.y - 4.0 * R2 * r2;
    ...
    let hit_pos = in.vertex_position + t * ray_dir_world;
