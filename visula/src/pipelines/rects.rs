...
impl Rects {
    pub fn new(
        ...
        let mut module =
            naga::front::wgsl::parse_str(include_str!("../shaders/rect.wgsl")).unwrap();
        let mut binding_builder = BindingBuilder::new(&module, "vs_main", 1);

        log::info!("Injecting rect shader delegate");
        delegate.inject("rect", &mut module, &mut binding_builder);
        ...
        log::debug!("Validating rect shader\n{module:#?}");
        let info =
        ...
        log::debug!("Resulting rects shader code:\n{output_str}");
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("rects render pipeline"),
...
impl Renderable for Rects {
    fn render(
        ...
        log::trace!("Rect count {count:#?}");
        if count.is_none() {
            log::debug!("Empty rect buffer detected. Aborting render of rects.");
            return;
        }
