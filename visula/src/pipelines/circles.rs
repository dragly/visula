...
impl Circles {
    pub fn new(
        ...
        let mut module =
            naga::front::wgsl::parse_str(include_str!("../shaders/circle.wgsl")).unwrap();
        let mut binding_builder = BindingBuilder::new(&module, "vs_main", 1);

        log::info!("Injecting circle shader delegate");
        delegate.inject("circle", &mut module, &mut binding_builder);
        ...
        log::debug!("Validating circle shader\n{module:#?}");
        let info =
        ...
        log::debug!("Resulting circles shader code:\n{output_str}");
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("circles render pipeline"),
...
impl Renderable for Circles {
    fn render(
        ...
        log::trace!("Circle count {count:#?}");
        if count.is_none() {
            log::debug!("Empty circle buffer detected. Aborting render of circles.");
            return;
        }
